...
def calcVBZB(df_wea,df_io,ind):
    ...
    df_calcs['Abar'] = 0
    df_calcs['Cmet'] = 0
    # Alternatives Verfahren für alle WEA mit NH kleiner 50m
...
def calcAndEvaluateNoise(df_wea,df_io,outfile=None):
    ...
    for ind_io in df_io.index:
        ...
        elif df_io.loc[ind_io, 'GB'] - 1.45 < df_io.loc[ind_io, 'IRW'] and df_io.loc[ind_io, 'VB'] + 10 > df_io.loc[ind_io, 'IRW']:
...
def schallKonzeptFortePiano(df_wea,df_io, fileName):
    ...
    for ind in df_wea['NachtBetrieb'].isnull().index:
    ...
    while any(df_io['Bewertung'] == 'Nicht ok'):
        ...
        mode_index = df_inp.iloc[start_index:aus_index].index.get_loc(df_wea.loc[wea_ind, 'NachtBetrieb'])
        if mode_index < aus_index - 1:
            # Lauteste Anlage ein Mode leiser
            df_wea.loc[wea_ind, 'NachtBetrieb'] = df_inp.iloc[mode_index + 1].name
            df_wea.loc[wea_ind, 63:'deltaL'] = df_inp.iloc[mode_index + 1, :10].values
            df_wea.loc[wea_ind, 'NB kW'] = df_inp.iloc[mode_index + 1, -1]
...
def ExtraRundeSchallModes(df_wea,df_io,fileName):
    ...
    for wea_ind in df_wea.loc[df_wea['Object type'] == 'Neue WEA'].index:
        ...
        elif df_modes.index.get_loc(df_wea.loc[wea_ind, 'NachtBetrieb']) < aus_index  and \
            df_modes.index.get_loc(df_wea.loc[wea_ind, 'NachtBetrieb']) > start_index:
    ...
    if not df_io[df_io.loc[:,'VB']>df_io.loc[:,'IRW']-10].empty:
        df_io.loc[df_io.loc[:,'VB']>df_io.loc[:,'IRW']-10,'IRW Calc']=\
            df_io.loc[df_io.loc[:,'VB']>df_io.loc[:,'VB']>df_io.loc[:,'IRW']-10,'IRW']+1
    ...
    bestCombinations = pd.DataFrame(index=range(0,len(df_wea)),columns=range(0,6))
    ...
    for i, combination in enumerate(product(*[df_io_allModes[df_io_allModes.index.str.startswith(prefix)].index for prefix in prefixes])):
        ...
        if all(df_io['Bewertung']!=0):
            if data1.loc[list(combination),'kW'].sum()>bestCombinations.loc['kW'].min():
                ...
                bestCombinations.loc['kW',bestCombinations.loc['kW'].idxmin()] = data1.loc[list(combination),'kW'].sum()
    ...
    df_wea['NB kW'].sum()
    df_wea.loc[:,'NachtBetrieb':'deltaL'] = data1.loc[list(bestCombinations.loc[:,bestCombinations.loc['kW'].idxmax()])[:-1],'NachtBetrieb':'deltaL'].values
    df_wea.loc[:,'NB kW'] = data1.loc[list(bestCombinations.loc[:,bestCombinations.loc['kW'].idxmax()])[:-1],'kW'].values
    return df_wea